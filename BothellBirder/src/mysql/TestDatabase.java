...
public class TestDatabase extends MySQL {
	...
	public TestDatabase( String host, String port, String database, String username, String password ) {
	...
	public void runExamples() throws Exception {
		...
		try {
			...
			// Connect to DB
			_connect = DriverManager.getConnection("jdbc:mysql://" + _host + ":" + _port + "/" + _database + "?" + "user=" + _username + "&password=" + _password );
			// connect = DriverManager.getConnection("jdbc:mysql://74.54.145.98:3306/nofrobro_javaTest?user=nofrobro_test&password=james1");
			
			if( debug ) System.out.println("Connection to Database Successful!");
			...
			_preparedStatement = _connect.prepareStatement("INSERT INTO users VALUES( default, ?, ?, ?, ? )");
			...
			if( debug ) System.out.println("Added user successful!");
			
			
			// Can get results with prepared statements as well
			_preparedStatement = _connect.prepareStatement( "SELECT * FROM users" );
			...
			_preparedStatement = _connect.prepareStatement( "DELETE FROM users WHERE id=? ; " );
